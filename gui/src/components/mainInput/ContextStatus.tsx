...
import { TooltipContent } from "../gui/TooltipContent";
...
const ContextStatus = () => {
  ...
  const barColorClass =
    percent < 60 ? "bg-success" : percent < 80 ? "bg-warning" : "bg-error";
  ...
  return (
    <div className="cursor-pointer">
      <TooltipContent
        ...
        clickable
        content={
          <div className="flex cursor-default flex-col gap-0 text-left text-xs">
            <span className="inline-block">{`该会话上下文已使用 ${percent}%`}</span>
            {/*            {isPruned && (
              <span className="inline-block">
                {`Oldest messages are being removed.`}
              </span>
            )}*/}
            {history.length > 0 && percent >= 60 && (
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <div className={`h-2 w-2 rounded-full ${barColorClass}`} />
          上下文用量 {percent}%
        </div>
      </TooltipContent>
    </div>
