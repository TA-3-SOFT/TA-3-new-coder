...
export default function ContinueLogo({
  ...
  const [isLightTheme, setIsLightTheme] = useState(() => {
    ...
    const editorBg = getComputedStyle(document.documentElement)
      ...
      .trim();
    return editorBg === "#ffffff" || editorBg === "rgb(255, 255, 255)" ||
  ...
  const updateTheme = () => {
    const editorBg = getComputedStyle(document.documentElement)
      .getPropertyValue("--vscode-editor-background")
      .trim();
    const newIsLightTheme = editorBg === "#ffffff" || editorBg === "rgb(255, 255, 255)" ||
      (editorBg.startsWith("#") && parseInt(editorBg.slice(1), 16) > 0x808080);
  ...
  // 监听CSS变量变化（用于其他IDE或手动主题切换）
  useEffect(() => {
    // 创建一个MutationObserver来监听style属性变化
    ...
    // 监听document.documentElement的style变化
  ...
  // 根据主题选择不同的颜色
  const colors = {
    color0: isLightTheme ? "#02389D" : "#8B5CF6",
    color1: isLightTheme ? "#02389C" : "#A78BFA",
    color2: isLightTheme ? "#F9C003" : "#F59E0B",
    color3: isLightTheme ? "#F9C004" : "#D97706",
    color4: isLightTheme ? "#FFFFFF" : "#F3F4F6",
  };
  ...
  return (
    ...
    >
      <path
        fill={colors.color0}
        d="M121.6,112c-2.2-0.1-4,0-5.8-0.3c-4.5-0.5-7.7-2.9-9.6-6.9c-1.2-2.7-1.8-5.5-2.1-8.4c-0.6-5-0.3-9.9-0.3-14.9
	...
	c-22.8-0.1-42.7,18.6-42.8,42.6C117,182.5,134.9,202.6,159.6,202.8z"
      ...
      <path
        fill={colors.color1}
        d="M86.2,134.9c0,20.9,0,41.9,0,62.8c0,1.4,0,2.8,0.3,4.1c1,4.4,3.6,7.2,7.9,8.6c1.1,0.4,2.2,0.6,3.3,0.7
	...
	C86.3,134.9,86.3,134.9,86.2,134.9z"
      ...
      <path
        fill={colors.color2}
        d="M228.7,60c1.1,0.7,2.1,1.3,3,2.1c4.3,3.4,6.7,7.8,6.3,13.5c-0.4,4.9-2.7,8.8-6.5,11.8c-3.2,2.6-7,3.8-11,4.1
	...
	c2.7-1.2,5.5-2.2,8.4-2.3c8.4-0.3,16.5,4.4,17.9,13.8c0.7,4.9-1.1,8.9-4.8,12.1C229.6,59.1,229.1,59.4,228.7,60z"
      ...
      <path
        fill={colors.color3}
        d="M191,53.7c0,1,0,2.1,0,3.1c0,0.7,0.3,0.9,1,0.9c1.8,0,3.5,0,5.3,0c0.7,0,1.4,0.1,2.1,0.2c2.1,0.5,3.4,1.9,3.5,4
	...
	C190.9,53.7,190.9,53.7,191,53.7z"
      />
      <circle fill={colors.color0} cx="160.5" cy="161.5" r="30.5" />
      <circle fill={colors.color2} cx="166.4" cy="158" r="17.6" />
      <circle fill={colors.color4} cx="169.9" cy="154.5" r="7" />
    </svg>
  );
}
