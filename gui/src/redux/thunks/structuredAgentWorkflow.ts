...
- 在此过程中可以使用rag_knowledge_query工具来查询相关知识，帮助更好地理解需求的相关背景。项目可能对于一些字段有自己的命名规则，所以需要调工具获取相关知识。
- 只返回模板内的内容就行，不需要对任务进行总结整理，不需要多余的提问。
...
1. *必须*使用project_analysis工具来分析当前Maven项目的结构，禁止传递任何参数给该工具（都使用默认的）。
