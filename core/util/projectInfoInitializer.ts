...
`;
  ...
  private async performSingleAnalysis(
    ...
    let messages: ChatMessage[] = [
      {
        role: "system",
        content: `你是一个资深的软件架构师和技术专家，擅长分析项目结构、技术栈和开发模式。请基于提供的项目信息，给出专业、详细且实用的分析。你的所有回答必须使用中文，禁止使用英文。不要编写建议的内容，只提供客观分析。你可以使用提供的工具来获取更多项目信息。使用工具时请串行调用，不要并行调用工具.
...
请确保分析内容专业、实用。在开始分析前，你必须先使用工具获取项目信息。所有输出内容必须使用中文，禁止使用英文。不要编写建议的内容，只提供客观分析。`,
      },
      {
        role: "user",
